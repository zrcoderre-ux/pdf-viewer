...
async function saveLeaks() {
  ...
  toast(`Saved ${leaks.name} — ${edits.length} decision${edits.length === 1 ? "" : "s"} written` + (und ? `, ${und} row${und === 1 ? "" : "s"} still to answer` : "") + ". Double-click Apply Fixes.bat (or re-run PDF-Linker) to apply them to the files.", { ms: 6000 });
}
