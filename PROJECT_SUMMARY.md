...
2. Capture case-caption party from a `X v. Y` tail and strip it. The
   captured party (e.g. `Singleton`) is used as a Complaint disambiguator.
...
5. Non-destructively capture the filing-party label from the leading
   possessive: `"Defendant Blue Shield's Demurrer"` → `"Blue Shield"`;
...
1. **Target only** — `Demurrer to SAC`, `Opposition to Demurrer`,
   `Mot. to Strike`.
2. **PartyLabel only** — `Receiver's Opposition`, `Blue Shield's
...
- Complaints use case-caption party (not partyLabel) as the qualifier:
  `Singleton Complaint` vs `Complaint`.
...
- `BLUE SHIELD'S NOTICE OF DEMURRER AND DEMURRER TO PLAINTIFF'S SAC` →
  `Demurrer` (target: SAC)
...
- `DECLARATION OF VIKTORIIA HROSHOVYK IN SUPPORT OF PLAINTIFF'S
  OPPOSITION TO ...` → `Hroshovyk Decl. ISO Opp.`
...
- Three Oppositions (Receiver's to ex parte, Plf's to demurrer, Blue
  ...
  Opposition` / `Blue Shield's Opposition` (level 2 of the ladder).
